<template>
  <div class="brand-theme min-h-screen text-slate-900" :style="themeStyle">
    ...
    <!-- Marketplace Header -->
    <header class="sticky top-0 z-50 shadow-md" style="background:linear-gradient(90deg,rgba(240,236,214,1) 0%,rgba(246,225,109,1) 50%,rgba(245,149,15,1) 100%);">
      <!-- Top row -->
                    ...
                    <div v-for="line in cartLines" :key="line.id" class="flex items-center gap-3">
                      ...
                      <div class="flex-1">
                        ...
                        <p class="text-[0.7rem] text-slate-500">
                          {{ line.variant || t('Standard') }} · {{ t('Qty :quantity', { quantity: line.quantity }) }}
                        </p>
                      </div>
      ...
      <div class="relative" style="background:linear-gradient(90deg,rgba(240,236,214,1) 0%,rgba(246,225,109,1) 50%,rgba(245,149,15,1) 100%);">
        <div class="container mx-auto px-4">
          ...
          >
            <div class="container mx-auto px-4 py-8">
              <div class="grid grid-cols-1 gap-8 lg:grid-cols-5">
                <!-- 4 Columns of Links -->
                <div v-for="(section, idx) in selectedCategory.sections" :key="'section-' + idx" class="space-y-3">
                ...
                <!-- Subcategories -->
                <div v-if="selectedCategory.children && selectedCategory.children.length" class="space-y-3">
                  <h3 class="text-sm font-bold uppercase tracking-wider text-[#0f172a]">
                    <!-- {{ t('Subcategories') }} -->
                  </h3>
                  <ul class="space-y-2">
                    <li v-for="child in selectedCategory.children" :key="child.slug || child.name" class="flex items-center gap-2">
                      <img
                        v-if="child.image"
                        :src="child.image"
                        alt=""
                ...
                >
                  <img v-if="selectedCategory.promo.image" :src="selectedCategory.promo.image" alt="Featured" class="h-full w-full object-cover" />
    ...
    <footer class="border-t border-slate-200 bg-white/90">
      <div class="container-base grid gap-8 py-10 sm:grid-cols-2 lg:grid-cols-5">
        ...
        <div v-for="column in footerColumns" :key="column.title" class="space-y-2 text-sm text-slate-600">
          ...
          <Link v-for="link in column.links || []" :key="link.href + link.label" :href="link.href" class="block hover:text-slate-900">
            {{ link.label }}
          </Link>
        </div>
      </div>
      ...
      >
        ...
        <div class="mt-6 space-y-6">
          ...
          <div class="space-y-2 text-sm text-slate-600">
            ...
            <div class="mt-4">
              <p class="text-xs font-semibold uppercase tracking-[0.2em] text-slate-400">{{ t('Currency') }}</p>
              <select
                v-model="selectedCurrency"
                @change="onCurrencyChange"
...
onMounted(() => {
  ...
  onBeforeUnmount(() => {
    offStart(); offFinish(); offError(); offInvalid()
  })
})
