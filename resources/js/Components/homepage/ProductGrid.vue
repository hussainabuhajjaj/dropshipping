<template>
  <section class="space-y-3">
    <div class="flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
      <div>
        <p class="text-[0.62rem] font-bold uppercase tracking-[0.22em] text-[#ff6b35]">{{ subtitle }}</p>
        <h2 class="text-[1.08rem] font-black tracking-[-0.03em] text-slate-950 sm:text-[1.3rem]">{{ title }}</h2>
      </div>

      <div v-if="pills.length" class="flex gap-1 overflow-x-auto pb-1 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden sm:gap-1.5">
        <button
          v-for="pill in pills"
          :key="pill.key"
          type="button"
    ...
    <div class="grid grid-cols-2 gap-2 sm:gap-2.5 lg:grid-cols-4 xl:grid-cols-5">
      <ProductCard
      ...
      <div
        ...
        v-for="index in 4"
        :key="`skeleton-${index}`"
        class="overflow-hidden rounded-[1.25rem] border border-[#efe7da] bg-white p-2 shadow-[0_12px_30px_rgba(15,23,42,0.04)]"
      >
        <div class="aspect-[0.82] animate-pulse rounded-[1rem] bg-[#f3eee7]"></div>
        <div class="space-y-1.5 px-1 pb-1 pt-2.5">
          ...
          <div class="h-9 animate-pulse rounded-full bg-[#efe7da]"></div>
        </div>
      </div>
    </div>
