    ...
    <div class="space-y-6">
      ...
      <form class="space-y-4" @submit.prevent="submit">
        ...
        <div class="flex items-center justify-between text-sm">
          <label class="flex min-h-11 items-center gap-2 text-slate-600">
            <Checkbox v-model:checked="form.remember" name="remember" />
            {{ t('Remember me') }}
          </label>
          <Link :href="route('password.request')" class="inline-flex min-h-11 items-center text-slate-600 hover:text-slate-900">
            {{ t('Forgot password?') }}
          </Link>
        </div>
      ...
      <div class="grid gap-2">
        ...
        <a
          ...
          class="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border border-slate-200 px-3 py-2 text-sm font-semibold text-slate-700 transition hover:border-slate-300"
        >
      ...
      <div class="flex flex-wrap items-center justify-between gap-2 text-sm text-slate-600">
        <Link :href="route('register')" class="inline-flex min-h-11 items-center hover:text-slate-900">{{ t('Create an account') }}</Link>
        <Link :href="route('claim-account.create')" class="inline-flex min-h-11 items-center hover:text-slate-900">{{ t('Claim existing order') }}</Link>
      </div>
    </div>
  </GuestLayout>
</template>
