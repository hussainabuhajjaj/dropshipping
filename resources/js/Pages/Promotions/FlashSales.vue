  ...
  <StorefrontLayout>
    <div class="max-w-5xl mx-auto py-10 space-y-8">
      <div class="flex items-center justify-between">
        <div>
          <h1 class="text-2xl font-bold">{{ t('Flash sales') }}</h1>
          <p class="text-slate-500 text-sm">{{ t('Limited-time drops with urgent pricing.') }}</p>
        </div>
        <Link href="/promotions/deals" class="text-sm text-[#29ab87] font-semibold">
          {{ t('View all deals') }}
        </Link>
      </div>
      ...
      <div v-else class="text-center text-slate-500 py-10">{{ t('No active flash sales right now.') }}</div>
    </div>
  </StorefrontLayout>
</template>
