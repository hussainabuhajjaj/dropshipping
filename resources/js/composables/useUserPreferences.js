...
export function useUserPreferences() {
  ...
  const initializePreferences = () => {
    preferences.value = normalizePreferences(page.props.user_preferences || {})
  }
  ...
  const setCurrency = async (currency) => {
    ...
    try {
      ...
      await router.put('/api/user-preferences/currency', { currency }, {
        preserveScroll: true,
        onSuccess: (page) => {
          preferences.value = normalizePreferences(page.props.user_preferences || {})
        },
  ...
  const setLanguage = async (language) => {
    ...
    try {
      ...
      await router.put('/api/user-preferences/language', { language }, {
        preserveScroll: true,
        onSuccess: (page) => {
          preferences.value = normalizePreferences(page.props.user_preferences || {})
        },
  ...
  const convertCurrency = (amount, fromCurrency, toCurrency) => {
    ...
    if (fromRate && toRate && typeof fromRate === 'number' && typeof toRate === 'number') {
  ...
  const getLocaleOptions = computed(() => {
    const userLocales = page.props.user_preferences?.available_languages || {}
  ...
  return {
    ...
    // Available options
    availableCurrencies: computed(() => preferences.value.availableCurrencies),
    availableLanguages: computed(() => preferences.value.availableLanguages),
