{{-- Test Progress Indicator - Shows for debugging --}}
@php
// Create test data to show the progress indicator
$activeImportTrackingKey = 'test_cj_import_' . time();
$importPercent = 65;
$importTotal = 100;
$importProcessed = 65;
$importFailed = 2;
$importStatusLabel = 'processing';
$importStartedAt = date('Y-m-d H:i:s', time() - 120); // Started 2 minutes ago
@endphp

@if ($activeImportTrackingKey)
    {{-- Custom Progress Indicator - Design 1: Modern Circular Progress --}}
    <div wire:poll.{{ $this->getImportPollIntervalSeconds() ?? 3 }}s="refreshQueueImportStatus" 
        ...
        <div class="absolute top-2 right-2">
            <span class="rounded-full bg-red-500 px-2 py-1 text-xs font-bold text-white">TEST MODE</span>
        </div>
            ...
            <div class="flex-1 pl-6">
                ...
                <div class="mt-3 text-xs text-gray-500 dark:text-gray-400">
                    @if ($importProcessed > 0)
                        ...
                        $rate = $importProcessed / max(1, microtime(true) - strtotime($importStartedAt));
...
@else
    <div class="rounded-lg border border-gray-200/70 bg-gray-50 p-3 dark:border-gray-800 dark:bg-gray-900/30">
        <p class="mt-1 text-center text-xs text-gray-500 dark:text-gray-500">
            No active imports - Start an import to see the progress indicator
        </p>
    </div>
@endif
