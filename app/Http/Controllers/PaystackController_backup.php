...
class PaystackController extends Controller
{
    ...
    public function initialize(Request $request): JsonResponse
    {
        ...
        try {
            ...
            $this->persistProviderResponse($payment, 'paystack_initialize', $result);
            ...
            return response()->json([
                'status' => false,
                'message' => $this->formatPaystackExceptionMessage($exception, $validated ?? []),
                'debug' => [
                    'file' => $exception->getFile(),
                    'line' => $exception->getLine(),
                    'trace' => collect(explode("\n", $exception->getTraceAsString()))->take(10)->toArray(),
                ],
            ], $this->resolveExceptionStatus($exception));
        }
    }
    ...
    public function verify(Request $request): JsonResponse
    {
        ...
        $reference = (string) $validator->validated()['reference'];

        try {
            $payment = $this->paymentService->verifyPaystack($reference)->load('order');
            $provider = $this->paystackService->verifyTransaction($reference);
            ...
            return response()->json([
                ...
                'data' => [
                    ...
                    'pending_confirmation' => $payment->status !== 'paid',
                    'display_text' => $provider['message'] ?? null,
                ],
            ]);
        } catch (\Throwable $exception) {
            ...
            return response()->json([
                'status' => false,
                'message' => $this->formatPaystackExceptionMessage($exception),
            ], $this->resolveExceptionStatus($exception));
        }
    }

    public function mobileMoneyCharge(Request $request): JsonResponse
    {
        $validator = Validator::make($request->all(), [
            'reference' => ['required', 'string', 'max:255'],
        ...
        try {
            ...
            $result = $this->paystackService->submitMobileMoneyOtp(
                $validated['reference'],
                $validated['otp']
            );

            $this->persistProviderResponse($payment, 'paystack_submit_otp', $result);
            ...
            Log::error('Paystack submit OTP failed', [
    ...
    private function formatPaystackExceptionMessage(\Throwable $exception, array $context = []): string
    {
        $message = trim((string) $exception->getMessage());

        if ($exception instanceof ApiException && is_array($exception->body)) {
            $providerMessage = trim((string) (
                data_get($exception->body, 'data.message')
                ?: data_get($exception->body, 'data.gateway_response')
                ?: data_get($exception->body, 'message')
                ?: ''
            ));
        ...
        if (str_contains($message, 'Customer email is required')) {
            return 'Email address is required to start payment.';
        }

        if (str_contains($message, 'Phone number is required')) {
            return 'Phone number is required for mobile money payments.';
        }

        if (str_contains($message, 'Payment reference is missing')) {
            return 'Payment reference error. Please try again.';
        }

        if (str_contains($message, 'Cart is empty')) {
            return 'Your cart is empty. Please add items to your cart.';
        }

        if (str_contains($message, 'Paystack did not return a valid authorization URL')) {
            return 'Payment initialization failed. Please try again.';
        }

        if (str_contains(strtolower($message), 'test mobile money number')) {
            $provider = strtolower((string) ($context['mobile_provider'] ?? ''));
            $phone = preg_replace('/\D+/', '', (string) ($context['phone'] ?? ''));
            $nextStep = trim((string) (
                $exception instanceof ApiException && is_array($exception->body)
                    ? data_get($exception->body, 'meta.nextStep', '')
                    : ''
            ));
            ...
            if ($provider === 'orange') {
                if ($phone === '0700000000') {
                    return $nextStep !== ''
                        ? 'Paystack rejected the official Orange CIV test number for this test key. ' . $nextStep
                        : 'Paystack rejected the official Orange CIV test number for this test key. This is an upstream Paystack account restriction and must be escalated through Paystack support.';
                }

                return 'Paystack test mode requires the official Orange CIV test number 0700000000. Use OTP 1234 after authorization.';
            }

            return 'Paystack test mode requires an official test mobile money number for this provider.';
        }

        return $message !== '' ? $message : 'Payment initialization failed. Please try again.';
    }

    private function resolveExceptionStatus(\Throwable $exception): int
    {
        if ($exception instanceof ApiException && is_int($exception->status) && $exception->status >= 400) {
    ...
    private function persistProviderResponse(Payment $payment, string $key, array $data): void
    {
        $payment->update([
            'meta' => array_merge($payment->meta ?? [], [
                $key => $data,
                $key . '_at' => now()->toISOString(),
            ]),
        ]);
    }
}
