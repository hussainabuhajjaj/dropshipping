...
class HighestMarginProductsTable extends BaseWidget
{
    ...
    protected function getTableQuery(): Builder|Relation|null
    {
        ...
        $allocatedCjShippingSql = "CASE WHEN COALESCE(orders.grand_total, 0) > 0 THEN (COALESCE(order_items.total, 0) / orders.grand_total) * COALESCE(orders.supplier_cj_shipping_total, 0) ELSE 0 END";
        $profitSql = "order_items.total - ((order_items.quantity * COALESCE(product_variants.cost_price, products.cost_price, 0)) + (order_items.quantity * {$externalShippingSql}) + {$allocatedCjShippingSql})";

        return OrderItem::query()
            ->selectRaw("order_items.product_variant_id, SUM(order_items.quantity) as units, SUM(order_items.total) as revenue, SUM({$profitSql}) as gross_profit, CASE WHEN SUM(order_items.total) > 0 THEN (SUM({$profitSql}) / SUM(order_items.total)) * 100 ELSE 0 END as gross_margin_percent")
