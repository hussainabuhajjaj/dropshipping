...
CREATE PROCEDURE calculate_cj_product_pricing(
...
BEGIN
    ...
    IF p_start_id IS NULL THEN SET p_start_id = 0; END IF;
    IF p_end_id IS NULL THEN SET p_end_id = 0; END IF;
    IF p_currency IS NULL THEN SET p_currency = 'USD'; END IF;
    IF p_margin_percentage IS NULL THEN SET p_margin_percentage = 30.00; END IF;
    IF p_force_update IS NULL THEN SET p_force_update = FALSE; END IF;
    IF p_chunk_size IS NULL OR p_chunk_size <= 0 THEN SET p_chunk_size = 1000; END IF;
